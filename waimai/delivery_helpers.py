...
from .models import ShopDeliverySettings
...
def calc_distance_fee(settings, distance_km, subtotal):
    ...
    max_km = float(settings.max_distance_km)
    if distance_km > max_km:
        return None

    rate = settings.per_km_rate_0_3
    if distance_km <= 3:
        fee = Decimal(str(distance_km)) * rate
        segment = f'{distance_km} 公里 × {rate} 元/公里'
    elif distance_km <= 6:
        fee = Decimal('3') * rate + Decimal(str(distance_km - 3)) * rate * settings.multiplier_3_6
        segment = (
            f'前3公里 {3 * rate} 元 + 超出 {distance_km - 3:.1f} 公里 × '
            f'{rate}×{settings.multiplier_3_6} 元/公里'
        )
    else:
        fee = (
            Decimal('3') * rate
            + Decimal('3') * rate * settings.multiplier_3_6
            + Decimal(str(distance_km - 6)) * rate * settings.multiplier_6_9
        )
        segment = (
            f'前3公里 {3 * rate} 元 + 3～6公里段 + 超出 {distance_km - 6:.1f} 公里 × '
            f'{rate}×{settings.multiplier_6_9} 元/公里'
        )

    fee = max(fee, settings.min_delivery_fee)
